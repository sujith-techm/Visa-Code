...
public class OnsiteArrival extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try{
			con=new Database();
			Qry="select max(OnSiteID) from OnsiteArrival";
			...
			if(!rs.next()){
				...
				if(newID==null){
					...
					newID="Avl"+(Integer.parseInt(newID.substring(3,newID.length()))+1);
				}
			}
			con.closeResultSet();
		}catch(Exception e){
			System.out.println("Error While Generating Onsite Arrival ID : "+e);
		}

		//Inserting Into Tables
		try{
			int no;
			Qry="insert into OnsiteArrival values('"+newID+"','"+ApplnID+"','"+ExeID+"','"+ticNo+"','"+fliNo+"','"+from+"','"+via+"','"+to+"','"+date+"','"+time+"','"+sysDate+"')";
			con=new Database();
			no=con.setResultSet(Qry);
			Qry="update ApplicationEntry set OnsiteArrival='yes' where ApplnID='"+ApplnID+"'";
			no=con.setResultSet(Qry);
			con.closeSetResultSet();
		}catch(Exception e){
			System.out.println("Error While Inserting OnsiteArrival : "+e);
		}
		response.sendRedirect("HrManager/OnsiteArrival.jsp");
    }
